...
export const PrediccionInit = () => {
    ...
    // 1. Objeto de configuración para manejar los 3 estados de la tendencia
    const trendConfig = {
        alta: {
            color: "text-success",
            icon: "fa-solid fa-arrow-trend-up",
            translationKey: "proyecciones.tendenciaAlta",
            translationText: "proyecciones.tendenciaUp"
        },
        baja: {
            color: "text-danger",
            icon: "fa-solid fa-arrow-trend-down",
            translationKey: "proyecciones.tendenciaBaja",
            translationText: "proyecciones.tendenciaDown"
        },
        estable: {
            color: "text-warning", // Color para "estable"
            icon: "fa-solid fa-minus", // Ícono para "estable"
            translationKey: "proyecciones.tendenciaEstable",
            translationText: "proyecciones.tendenciaStable"
        },
    };

    const currentTrendConfig = trendConfig[trend] || trendConfig.estable;
                    ...
                    {data.isError ? <div className="alert alert-danger">{data.detail}</div> : (
                        <>
                            ...
                            <div className="row g-3 mb-4">
                                ...
                                <div className="col-md col-6">
                                    <div className="card p-3 shadow-sm">
                                        <div className="fw-bold">{t("proyecciones.tipoPrediccion")}</div>
                                        <div className="h4">{data.prediction_type === "Simple (Datos Insuficientes)"  ? (t("proyecciones.prediccionBasica")) : (t("proyecciones.prediccionRobusta"))}</div>
                                        <small className="text-muted">{data.prediction_type === "Simple (Datos Insuficientes)"  ? (t("proyecciones.modeloSimple")) : (t("proyecciones.modeloML"))}</small>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div className="card shadow-sm">
                                        ...
                                        <div className="card-body">
                                            <div className="d-flex align-items-center gap-2 mb-2">
                                                {/* 3. El JSX ahora usa la configuración dinámica */}
                                                <span className={`fw-medium ${currentTrendConfig.color}`}>
                                                    <i className={currentTrendConfig.icon}></i>
                                                    {t("proyecciones.tendencia")}: {t(currentTrendConfig.translationKey)}
                                                </span>
                                            </div>
                                            <p className="text-muted small mb-0">
                                                {/* 4. El texto también es dinámico */}
                                                {t(currentTrendConfig.translationText)}
                                            </p>
                                        </div>
                                    </div>
                                </div>
